...
implements Cloneable {
    ...
    public List<Message> extra = new ArrayList<Message>();
    ...
    public static Message fromJson(JsonElement e) {
        ...
        if (e.isJsonObject()) {
            ...
            if (json.has("text")) {
                ...
                if (json.has("with")) {
                    ChatFormat[] withJson = json.get("with").getAsJsonArray();
            ...
            for (ChatFormat format : ChatFormat.values()) {
                if (!json.has(format.toString()) || !json.get(format.toString()).getAsBoolean()) continue;
