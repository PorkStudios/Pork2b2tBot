...
public final class ShortArrays {
    ...
    public static void mergeSort(short[] a, int from, int to) {
        ShortArrays.mergeSort(a, from, to, (short[])a.clone());
    }
    ...
    public static void mergeSort(short[] a, int from, int to, ShortComparator comp) {
        ShortArrays.mergeSort(a, from, to, comp, (short[])a.clone());
    }
    ...
    public static void radixSortIndirect(int[] perm, short[] a, int from, int to, boolean stable) {
        int[] support;
        if (to - from < 1024) {
            ShortArrays.insertionSortIndirect(perm, a, from, to);
            return;
        }
        boolean maxLevel = true;
        int stackSize = 256;
        ...
        while (stackPos > 0) {
            ...
            if (stable) {
                ...
                for (i = 0; i <= lastUsed; ++i) {
                    if (level < 1 && count[i] > 1) {
                        if (count[i] < 1024) {
                            ShortArrays.insertionSortIndirect(perm, a, p, p + count[i]);
                        } else {
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                ...
                if (level < 1 && count[c] > 1) {
                    if (count[c] < 1024) {
                        ShortArrays.insertionSortIndirect(perm, a, i3, i3 + count[c]);
                    } else {
                        offsetStack[stackPos] = i3;
                        lengthStack[stackPos] = count[c];
                        levelStack[stackPos++] = level + 1;
    ...
    public static void radixSort(short[] a, short[] b, int from, int to) {
        if (to - from < 1024) {
            ShortArrays.selectionSort(a, b, from, to);
            return;
        }
        int layers = 2;
        int maxLevel = 3;
        ...
        while (stackPos > 0) {
            ...
            for (int i3 = first; i3 <= end; i3 += count[c]) {
                ...
                if (level < 3 && count[c] > 1) {
                    if (count[c] < 1024) {
                        ShortArrays.selectionSort(a, b, i3, i3 + count[c]);
                    } else {
                        offsetStack[stackPos] = i3;
                        lengthStack[stackPos] = count[c];
                        levelStack[stackPos++] = level + 1;
