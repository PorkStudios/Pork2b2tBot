...
Hash {
    ...
    public ObjectLinkedOpenCustomHashSet<K> clone() {
        ...
        try {
            c = (ObjectLinkedOpenCustomHashSet)Object.super.clone();
        }
    ...
    implements ObjectListIterator<K> {
        ...
        SetIterator(K from) {
            ...
            int pos = HashCommon.mix(ObjectLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & ObjectLinkedOpenCustomHashSet.this.mask;
            while (key[pos] != null) {
        ...
        public void remove() {
            ...
            if (this.prev == -1) {
                ...
                long[] arrl = ObjectLinkedOpenCustomHashSet.this.link;
                int n = this.prev;
                arrl[n] = arrl[n] ^ (ObjectLinkedOpenCustomHashSet.this.link[this.prev] ^ (long)this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
            }
            if (this.next == -1) {
                ...
                long[] arrl = ObjectLinkedOpenCustomHashSet.this.link;
                int n = this.next;
                arrl[n] = arrl[n] ^ (ObjectLinkedOpenCustomHashSet.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & -4294967296L;
            }
            int pos = this.curr;
            this.curr = -1;
            if (pos != ObjectLinkedOpenCustomHashSet.this.n) {
                ...
                do {
                    Object curr;
                    int last = pos;
                    pos = last + 1 & ObjectLinkedOpenCustomHashSet.this.mask;
                    do {
                        if ((curr = key[pos]) == null) {
                            key[last] = null;
                            return;
