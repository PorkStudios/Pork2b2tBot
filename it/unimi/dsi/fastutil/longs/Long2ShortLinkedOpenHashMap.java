...
Hash {
    ...
    public short computeIfPresent(long k, BiFunction<? super Long, ? super Short, ? extends Short> remappingFunction) {
        ...
        Short newValue = remappingFunction.apply((Long)k, (Short)this.value[pos]);
        if (newValue == null) {
            if (k == 0L) {
                this.removeNullEntry();
            } else {
    ...
    public short compute(long k, BiFunction<? super Long, ? super Short, ? extends Short> remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        int pos = this.find(k);
        Short newValue = remappingFunction.apply((Long)k, pos >= 0 ? Short.valueOf(this.value[pos]) : null);
        if (newValue == null) {
            if (pos >= 0) {
                if (k == 0L) {
                    this.removeNullEntry();
            ...
            this.values = new AbstractShortCollection(){
                ...
                public void forEach(IntConsumer consumer) {
                    if (Long2ShortLinkedOpenHashMap.this.containsNullKey) {
                        consumer.accept(Long2ShortLinkedOpenHashMap.this.value[Long2ShortLinkedOpenHashMap.this.n]);
                    }
                    int pos = Long2ShortLinkedOpenHashMap.this.n;
                    while (pos-- != 0) {
                        if (Long2ShortLinkedOpenHashMap.this.key[pos] == 0L) continue;
                        consumer.accept(Long2ShortLinkedOpenHashMap.this.value[pos]);
                    }
                }
            };
        }
    ...
    public Long2ShortLinkedOpenHashMap clone() {
        ...
        try {
            c = (Long2ShortLinkedOpenHashMap)Object.super.clone();
        }
    ...
    extends AbstractLongSortedSet {
        ...
        public void forEach(LongConsumer consumer) {
            ...
            int pos = Long2ShortLinkedOpenHashMap.this.n;
            while (pos-- != 0) {
                long k = Long2ShortLinkedOpenHashMap.this.key[pos];
                if (k == 0L) continue;
                consumer.accept(k);
            }
        }
    ...
    implements Long2ShortSortedMap.FastSortedEntrySet {
        ...
        public boolean contains(Object o) {
            ...
            int pos = (int)HashCommon.mix(k) & Long2ShortLinkedOpenHashMap.this.mask;
            long curr = key[pos];
            if (curr == 0L) {
                return false;
            }
            ...
            do {
                if ((curr = key[pos = pos + 1 & Long2ShortLinkedOpenHashMap.this.mask]) != 0L) continue;
                return false;
            } while (k != curr);
            return Long2ShortLinkedOpenHashMap.this.value[pos] == v;
        }
        ...
        public boolean remove(Object o) {
            ...
            do {
                if ((curr = key[pos = pos + 1 & Long2ShortLinkedOpenHashMap.this.mask]) != 0L) continue;
                return false;
            } while (curr != k || Long2ShortLinkedOpenHashMap.this.value[pos] != v);
            Long2ShortLinkedOpenHashMap.this.removeEntry(pos);
            return true;
        }
        ...
        public void forEach(Consumer<? super Long2ShortMap.Entry> consumer) {
            if (Long2ShortLinkedOpenHashMap.this.containsNullKey) {
                consumer.accept(new AbstractLong2ShortMap.BasicEntry(Long2ShortLinkedOpenHashMap.this.key[Long2ShortLinkedOpenHashMap.this.n], Long2ShortLinkedOpenHashMap.this.value[Long2ShortLinkedOpenHashMap.this.n]));
            }
            int pos = Long2ShortLinkedOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Long2ShortLinkedOpenHashMap.this.key[pos] == 0L) continue;
                consumer.accept(new AbstractLong2ShortMap.BasicEntry(Long2ShortLinkedOpenHashMap.this.key[pos], Long2ShortLinkedOpenHashMap.this.value[pos]));
            }
        }
        ...
        public void fastForEach(Consumer<? super Long2ShortMap.Entry> consumer) {
            ...
            if (Long2ShortLinkedOpenHashMap.this.containsNullKey) {
                entry.key = Long2ShortLinkedOpenHashMap.this.key[Long2ShortLinkedOpenHashMap.this.n];
                entry.value = Long2ShortLinkedOpenHashMap.this.value[Long2ShortLinkedOpenHashMap.this.n];
                consumer.accept(entry);
            }
            int pos = Long2ShortLinkedOpenHashMap.this.n;
            while (pos-- != 0) {
                if (Long2ShortLinkedOpenHashMap.this.key[pos] == 0L) continue;
                entry.key = Long2ShortLinkedOpenHashMap.this.key[pos];
                entry.value = Long2ShortLinkedOpenHashMap.this.value[pos];
                consumer.accept(entry);
            }
        }
    ...
    private class MapIterator {
        ...
        private MapIterator(long from) {
            ...
            int pos = (int)HashCommon.mix(from) & Long2ShortLinkedOpenHashMap.this.mask;
            while (Long2ShortLinkedOpenHashMap.this.key[pos] != 0L) {
        ...
        public void remove() {
            ...
            if (this.prev == -1) {
                ...
                long[] arrl = Long2ShortLinkedOpenHashMap.this.link;
                int n = this.prev;
                arrl[n] = arrl[n] ^ (Long2ShortLinkedOpenHashMap.this.link[this.prev] ^ (long)this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
            }
            if (this.next == -1) {
                ...
                long[] arrl = Long2ShortLinkedOpenHashMap.this.link;
                int n = this.next;
                arrl[n] = arrl[n] ^ (Long2ShortLinkedOpenHashMap.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & -4294967296L;
            }
            int pos = this.curr;
            this.curr = -1;
            if (pos != Long2ShortLinkedOpenHashMap.this.n) {
                ...
                do {
                    long curr;
                    int last = pos;
                    pos = last + 1 & Long2ShortLinkedOpenHashMap.this.mask;
                    do {
                        if ((curr = key[pos]) == 0L) {
                            key[last] = 0L;
                            return;
                        }
                        int slot = (int)HashCommon.mix(curr) & Long2ShortLinkedOpenHashMap.this.mask;
                        if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos) break;
                        pos = pos + 1 & Long2ShortLinkedOpenHashMap.this.mask;
                    } while (true);
                    key[last] = curr;
