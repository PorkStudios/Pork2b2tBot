...
Cloneable {
    ...
    public ObjectSortedSet<Float2ObjectMap.Entry<V>> float2ObjectEntrySet() {
        if (this.entries == null) {
            this.entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<V>>(){
                final Comparator<? super Float2ObjectMap.Entry<V>> comparator = (x, y) -> Float2ObjectAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
                ...
                public boolean contains(Object o) {
                    ...
                    Entry f = Float2ObjectAVLTreeMap.this.findKey(((Float)e.getKey()).floatValue());
                    return e.equals(f);
                }
    ...
    public Float2ObjectAVLTreeMap<V> clone() {
        Float2ObjectAVLTreeMap c;
        try {
            c = (Float2ObjectAVLTreeMap)Object.super.clone();
        }
    ...
    implements Serializable {
        ...
        public V remove(float k) {
            ...
            Object oldValue = Float2ObjectAVLTreeMap.this.remove(k);
        ...
        public Float2ObjectSortedMap<V> subMap(float from, float to) {
            ...
            if (!this.top) {
                float f = to = Float2ObjectAVLTreeMap.this.compare(to, this.to) < 0 ? to : this.to;
            }
            if (!this.bottom) {
                float f = from = Float2ObjectAVLTreeMap.this.compare(from, this.from) > 0 ? from : this.from;
            }
        ...
        extends Float2ObjectAVLTreeMap<V>
        ...
        extends Float2ObjectAVLTreeMap<V> {
            ...
            SubmapIterator(float k) {
                ...
                if (!submap.bottom && submap.Float2ObjectAVLTreeMap.this.compare(k, this.next.key) < 0) {
                    this.prev = null;
                    return;
                }
                if (!submap.top) {
                    this.prev = submap.lastEntry();
                    if (submap.Float2ObjectAVLTreeMap.this.compare(k, this.prev.key) >= 0) {
                        this.next = null;
                        return;
                    }
                }
                this.next = submap.Float2ObjectAVLTreeMap.this.locateKey(k);
                if (submap.Float2ObjectAVLTreeMap.this.compare(this.next.key, k) <= 0) {
                    this.prev = this.next;
