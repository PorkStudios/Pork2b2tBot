...
Hash {
    ...
    public ShortLinkedOpenHashSet clone() {
        ...
        try {
            c = (ShortLinkedOpenHashSet)Object.super.clone();
        }
    ...
    implements ShortListIterator {
        ...
        SetIterator(short from) {
            ...
            int pos = HashCommon.mix(from) & ShortLinkedOpenHashSet.this.mask;
            while (key[pos] != 0) {
                if (key[pos] == from) {
                ...
                pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
            }
            throw new NoSuchElementException("The key " + from + " does not belong to this set.");
        }
        ...
        public void remove() {
            ...
            if (this.prev == -1) {
                ...
                long[] arrl = ShortLinkedOpenHashSet.this.link;
                int n = this.prev;
                arrl[n] = arrl[n] ^ (ShortLinkedOpenHashSet.this.link[this.prev] ^ (long)this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
            }
            if (this.next == -1) {
                ...
                long[] arrl = ShortLinkedOpenHashSet.this.link;
                int n = this.next;
                arrl[n] = arrl[n] ^ (ShortLinkedOpenHashSet.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & -4294967296L;
            }
            int pos = this.curr;
            this.curr = -1;
            if (pos != ShortLinkedOpenHashSet.this.n) {
                ...
                do {
                    short curr;
                    int last = pos;
                    pos = last + 1 & ShortLinkedOpenHashSet.this.mask;
                    do {
                        if ((curr = key[pos]) == 0) {
                            key[last] = 0;
                            return;
