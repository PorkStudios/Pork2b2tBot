...
lbl159: // 1 sources:
    ...
    public Object clone() {
        ShortRBTreeSet c;
        try {
            c = (ShortRBTreeSet)Object.super.clone();
        }
    ...
    ShortSortedSet {
        ...
        public ShortSortedSet subSet(short from, short to) {
            ...
            if (!this.top) {
                short s = to = ShortRBTreeSet.this.compare(to, this.to) < 0 ? to : this.to;
            }
            if (!this.bottom) {
                short s = from = ShortRBTreeSet.this.compare(from, this.from) > 0 ? from : this.from;
            }
        ...
        extends SetIterator {
            ...
            SubsetIterator(short k) {
                ...
                if (!subset.bottom && subset.ShortRBTreeSet.this.compare(k, this.next.key) < 0) {
                    this.prev = null;
                    return;
                }
                if (!subset.top) {
                    this.prev = subset.lastEntry();
                    if (subset.ShortRBTreeSet.this.compare(k, this.prev.key) >= 0) {
                        this.next = null;
                        return;
                    }
                }
                this.next = subset.ShortRBTreeSet.this.locateKey(k);
                if (subset.ShortRBTreeSet.this.compare(this.next.key, k) <= 0) {
                    this.prev = this.next;
